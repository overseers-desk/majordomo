...
**Streaming is a use-case, not the foundation.** The cache is filled by the BI connector's hourly cursor sync, a windowed pull; majordomo reads the result. A live tail of incoming messages, an agent acting per message, is a further mode wanted by the automation ambition, and a daemon delivery (below) would subsume it. The data model does not require streaming; the sync cadence, not a live socket, is the freshness knob.

## Delivery: CLI or daemon over the cache

majordomo does not own the sync — the BI connector does, and the standalone fallback is a direct live read. What is open is how majordomo serves its own callers, with at least two shapes.
...
- **It is self-sufficient without that backend.** majordomo retains its own live Chat read and its own task decoder, so it runs where the BI platform is absent; reconstruction is reused from `coord_tasks` when present and performed by majordomo's own decoder when not. The backend accelerates, it does not gate.
...
- **The sieve, identity-keyed multi-account, reporting, and the CLI/MCP front doors are majordomo's own work**, over whichever source — BI cache or live read — serves the flow.
...
This settles that majordomo is a reader, reporter, and gate that uses the BI platform's chat cache and task reconstruction as a fast path while keeping its own live read and decoder for standalone use. The `DESIGN.md` question of whether to fork `gchat-cli` now applies to that standalone live path, and still rests on the pagination check recorded there.
