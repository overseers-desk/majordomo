...
| `people` | Aggregate over `googlechat_messages` union `coord_tasks` (cache) / same aggregation over API messages | The counts are append-only derived and boundable exactly. The display names are mutable current-state: cache-side they come from `googlechat_users.display_name` (a current-state table) with the prose `@name` as fallback; API-side from the prose `@name` frozen in the message text (that one is actually as-of-creation, so exact). |
...
- **`people`, both backends**: the aggregation windows are already `start`/`end` parameters; clamping `end` bounds the counts exactly, server-side in cache, at the message fetch in the API path. Cache-side display names get flagged, not rewound (rule 3).
...
Three places cannot be rewound, and the rule is to say so rather than pretend:
