...
A read uses the server-side cache by default and falls back to the live Chat API when the cache is unreachable. `--cache` or `--live` before the command forces one source; `--live` needs the live extra and a prior `majordomo login`.
